...
print('Программа для импорта фотографий из ВКонтаке на Яндекс Диск')
...
user_id = input('Введите свой ID или никнейм страницы профиля: ')
...
class VK_API:
    ...
    def file_name(res):
        ...
        # условие: если количество лайков под фотографиями одинаковое, то им добавляются даты загрузки
        ...
        # если количество лайков разное - то именабудут состоять только из количества лайков
        else:
            return get_list_likes
        ...
        for item in res.values():
            for inside_params in item['items']:
                ...
                for size in inside_params['sizes']:
                    ...
                    url_list.append(url)
                    # создание словаря, чтобы по значению выводить наибольшее значение
                    dict_for_info = dict(zip(url_list, size_list))
                    # привязываемся по дате фотографий с указанием типоразмера
